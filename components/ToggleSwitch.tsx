  ...
  return (
    <label htmlFor={id} className="flex items-center cursor-pointer group">
      <div className="relative">
        ...
        />
        {/* Track */}
        <div className={`block w-14 h-8 rounded-full transition-colors duration-200 ${checked ? 'bg-amber-500' : 'bg-slate-300 group-hover:bg-slate-400 dark:bg-slate-600 dark:group-hover:bg-slate-500'}`}></div>
        {/* Thumb */}
      ...
      <div className="ml-4 text-slate-700 dark:text-slate-200 font-medium">
        {children}
      </div>
    </label>
  );
